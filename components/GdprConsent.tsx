...
export default function GdprConsent() {
    ...
    const accept = () => {
        try {
            window.localStorage.setItem(KEY, JSON.stringify({t: Date.now(), v: 1}))
        } catch {
        }
        setOpen(false)
    }
    const reject = () => {
        try {
            window.localStorage.setItem(KEY, JSON.stringify({t: Date.now(), v: 1, rejected: true}))
        } catch {
        }
        setOpen(false)
    }
    ...
    return (
      ...
      dark:bg-zinc-900/80 backdrop-blur-xl p-4 pointer-events-auto animate-[fadeIn_.4s_ease]">
                ...
                <p className="text-xs leading-relaxed text-zinc-700 dark:text-zinc-300 mb-3">
                    Lorem ipsum dolor sit amet, consectetur adipiscing elit. Etiam euismod, sapien quis facilisis
                    dictum, augue
                    sem pharetra purus, non vulputate lectus nulla at elit. Donec at lacus sed ipsum fermentum placerat.
                    Nulla
                ...
                <div className="flex flex-wrap gap-2 justify-end">
                    <button onClick={reject} className="px-3 py-1.5 text-xs font-medium rounded-full border border-zinc-300
          ...
          focus-visible:ring-pink-500/50"
                    ...
                    <button onClick={accept} className="px-4 py-1.5 text-xs font-semibold rounded-full bg-pink-600
