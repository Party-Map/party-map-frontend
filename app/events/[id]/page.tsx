    ...
    return (
        <>
            ...
            <main className="pt-24 px-4 pb-24 md:pb-0">
                ...
                <div
                    className="mt-2 overflow-hidden rounded-2xl border border-gray-300/80 dark:border-white/10 bg-white/90 dark:bg-zinc-950/80 backdrop-blur shadow-sm">
                    <Image src={event.image} alt={event.title} width={1600} height={900}
                    ...
                    <div className="p-4">
                        ...
                        {place && (
                            <p className="text-sm text-zinc-600 dark:text-zinc-300">
                                at <Link href={`/places/${place.id}`}
                                         className="text-violet-600 dark:text-violet-300 hover:underline">{place.name}</Link>
                            </p>
                        )}
                        ...
                        {performerEntities.filter(Boolean).map(p => (
                            <li key={p!.id}
                                ...
                                <Link href={`/performers/${p!.id}`}
                                      className="text-violet-600 dark:text-violet-300 hover:underline font-medium">{p!.name}</Link>
